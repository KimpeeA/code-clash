...
@export var running_speed: float = 400 * 1.6
@export var distance: int = 40
...
func update_jump_properties(
	p_jump_height: float, p_jump_time_to_peak: float, p_jump_time_to_descent: float
) -> void:
	...
	fall_gravity = (
		((-2.0 * p_jump_height) / (p_jump_time_to_descent * p_jump_time_to_descent)) * -1.0
	)
