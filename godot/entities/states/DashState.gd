...
func _on_enter(args) -> void:
	...
	var distance := _agent._stats.dash_distance
	var duration := _agent._stats.dash_duration
	if duration <= 0.0:
...
func _on_update(delta: float) -> void:
	print(
		"vel: {}, prev_vel: {}, face_dir: {}, dir: {}".format(
			[_agent.velocity, _previous_velocity, _agent._facing_direction, _dir], "{}"
		)
	)
